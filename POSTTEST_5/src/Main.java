...
void main() {
    ...
    while (programUtamaJalan) {
        ...
        IO.println("   Posttest 4 - Polymorphism (Overload & Override)");
        IO.println("   Oleh: Febrian Pratama Saputra (2409106033)    ");
        IO.println("=================================================");
        IO.println("1. Kelola Data Siswa (Test Overloading 1)");
        IO.println("2. Kelola Data Instruktur (Test Overloading 2)");
        IO.println("3. Kelola Data Kendaraan (Test Overriding 1 & 2)");
        ...
        switch (pilihanUtama) {
            case 1:
                ...
                while (menuSiswaJalan) {
                    ...
                    switch (pilSiswa) {
                        case 1:
                            IO.print("ID Siswa: "); String id = scanner.nextLine();
                            IO.print("Nama Siswa: "); String nama = scanner.nextLine();
                            IO.print("Paket (Manual/Matic): "); String paket = scanner.nextLine();
                        ...
                        case 2:
                            ...
                            break;
                        case 3:
                            IO.print("Masukkan ID Siswa yang diubah: "); String idEdit = scanner.nextLine();
                            boolean foundSiswa = false;
                            for (SiswaMengemudi s : daftarSiswa) {
                                if (s.getIdSiswa().equals(idEdit)) {
                                    IO.print("Nama Baru: "); s.setNama(scanner.nextLine());
                                    IO.print("Paket Baru: "); s.setPaketKursus(scanner.nextLine());
                                    IO.println("Data diperbarui.");
                                    foundSiswa = true; break;
                                }
                            }
                            if (!foundSiswa) IO.println("ID tidak ditemukan.");
                            break;
                        case 4:
                            IO.print("Masukkan ID Siswa: "); String idJadwal = scanner.nextLine();
                            boolean foundJadwal = false;
                            for (SiswaMengemudi s : daftarSiswa) {
                                if (s.getIdSiswa().equals(idJadwal)) {
                                    IO.print("Masukkan Hari: "); String hari = scanner.nextLine();
                                    IO.print("Masukkan Jam (Kosongkan jika bebas/fleksibel): "); String jam = scanner.nextLine();

                                    if (jam.isEmpty()) {
                                        s.aturJadwal(hari);
                                    } else {
                                        s.aturJadwal(hari, jam);
                                    }
                                    IO.println("Jadwal diatur.");
                                    foundJadwal = true; break;
                                }
                            }
                            if (!foundJadwal) IO.println("ID tidak ditemukan.");
                            break;
                        case 5:
                            IO.print("Masukkan ID Siswa yang dihapus: "); String idDel = scanner.nextLine();
                            daftarSiswa.removeIf(s -> s.getIdSiswa().equals(idDel));
                            IO.println("Eksekusi hapus selesai.");
                            break;
                        case 6: menuSiswaJalan = false; break;
                    }
                }
                break;
            ...
            case 2:
                ...
                while (menuInstrukturJalan) {
                    ...
                    IO.println("1. Tambah Instruktur\n2. Lihat Instruktur\n3. Ubah Instruktur\n4. Tambah Beban Ajar (Overloading)\n5. Hapus Instruktur\n6. Kembali");
                    IO.print("Pilih: ");
                    ...
                    switch (pilInstruktur) {
                        case 1:
                            IO.print("ID Instruktur: "); String id = scanner.nextLine();
                            IO.print("Nama Instruktur: "); String nama = scanner.nextLine();
                            IO.print("Lisensi (A/B/C): "); String lisensi = scanner.nextLine();
                        ...
                        case 2:
                            ...
                            break;
                        case 3:
                            IO.print("Masukkan ID Instruktur yang diubah: "); String idEdit = scanner.nextLine();
                            boolean foundInst = false;
                            for (Instruktur inst : daftarInstruktur) {
                                if (inst.getIdInstruktur().equals(idEdit)) {
                                    IO.print("Nama Baru: "); inst.setNama(scanner.nextLine());
                                    IO.print("Lisensi Baru: "); inst.setLisensi(scanner.nextLine());
                                    IO.println("Data diperbarui.");
                            ...
                            break;
                        case 4:
                            IO.print("Masukkan ID Instruktur: "); String idAjar = scanner.nextLine();
                            boolean foundAjar = false;
                            for (Instruktur inst : daftarInstruktur) {
                                if (inst.getIdInstruktur().equals(idAjar)) {
                                    IO.print("Ketik jumlah siswa yang ditambahkan (Biarkan kosong untuk tambah 1): ");
                                    String inputTambahan = scanner.nextLine();

                                    if (inputTambahan.isEmpty()) {
                                        inst.tambahSiswa();
                                    } else {
                                        inst.tambahSiswa(Integer.parseInt(inputTambahan));
                                    }
                                    IO.println("Beban ajar diperbarui.");
                                    foundAjar = true; break;
                                }
                            }
                            if (!foundAjar) IO.println("ID tidak ditemukan.");
                            break;
                        case 5:
                            IO.print("Masukkan ID Instruktur yang dihapus: "); String idDel = scanner.nextLine();
                            daftarInstruktur.removeIf(i -> i.getIdInstruktur().equals(idDel));
                            IO.println("Eksekusi hapus selesai.");
                            break;
                        case 6: menuInstrukturJalan = false; break;
                    }
                }
                break;
                ...
                while (menuKendaraanJalan) {
                    ...
                    switch (pilKendaraan) {
                        case 1:
                            ...
                            IO.print("Pilihan (1-3): "); int jenisKendaraan = scanner.nextInt(); scanner.nextLine();
                            IO.print("Plat Nomor: "); String plat = scanner.nextLine();
                            IO.print("Merk Kendaraan: "); String merk = scanner.nextLine();

                            if (jenisKendaraan == 1) {
                                IO.print("Kondisi Kopling (Baru/Aus): "); daftarKendaraan.add(new MobilManual(plat, merk, scanner.nextLine()));
                            } else if (jenisKendaraan == 2) {
                                IO.print("Status CVT: "); daftarKendaraan.add(new MobilMatic(plat, merk, scanner.nextLine()));
                            } else if (jenisKendaraan == 3) {
                                IO.print("Kapasitas (Ton): "); daftarKendaraan.add(new TrukInstruksi(plat, merk, scanner.nextInt())); scanner.nextLine();
                        ...
                        case 3:
                            IO.print("Masukkan Plat Nomor yang diubah: "); String platEdit = scanner.nextLine();
                            boolean foundKendaraan = false;

                            for (KendaraanKursus k : daftarKendaraan) {
                                if (k.getPlatNomor().equals(platEdit)) {
                                    IO.print("Merk Baru: "); k.setMerk(scanner.nextLine());
                        ...
                        case 4:
                            IO.print("Masukkan Plat Nomor yang dihapus: "); String platDel = scanner.nextLine();
                            daftarKendaraan.removeIf(k -> k.getPlatNomor().equals(platDel));
                            IO.println("Eksekusi hapus selesai.");
                            break;
                        case 5: menuKendaraanJalan = false; break;
                    }
                }
                break;
